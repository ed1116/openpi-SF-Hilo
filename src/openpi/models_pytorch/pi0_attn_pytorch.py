...
class PI0Pytorch(nn.Module):
    ...
    def _prepare_taskaware_text(
        self, all_hidden_states, img_len: int, lang_masks: torch.Tensor, *, taskaware_needs_grad: bool = True,
        ...
        hidden_state_k = all_hidden_states[text_layer]
        # [COPILOT] Support both hidden-state layouts:
        # 1) full PI0 forward: ([prefix_hidden, suffix_hidden], ...)
        # 2) stage1 prefix-only fast path: (prefix_hidden, ...)
        if isinstance(hidden_state_k, (list, tuple)):
            prefix_hidden_k = hidden_state_k[0]
        else:
            prefix_hidden_k = hidden_state_k
        ...
        # [COPILOT] Skip text dropout when Q-Former is frozen (stage 2): dropout would only inject noise into the alignment target.
        if self.training and self.taskaware_text_dropout > 0.0 and taskaware_needs_grad:
            keep_text = (
    ...
    def forward(
        ...
        if compute_action_loss or compute_align_loss:
            ...
            # =================================== Stage 1 fast path ===================================
            # [COPILOT] Stage 1 only trains task-aware blocks. Skip suffix/action/align computation
        ...
        # Skip text dropout when task-aware branch is frozen or task loss is disabled.
        text_hidden, text_mask = self._prepare_taskaware_text(
            all_hidden_states,
            img_len,
            lang_masks,
            taskaware_needs_grad=(taskaware_needs_grad and compute_task_loss),
        )
        ...
        if compute_task_loss:
            ...
            taskaware_aux_loss = actions.new_zeros(())
