...
def _add_2d_positional_embedding(
    tokens: torch.Tensor,
    grid_hw: tuple[int, int],
    img_hw: tuple[int, int],
    ratio: float = 0.1,
) -> torch.Tensor:
    """Add the same UV-grid sinusoidal 2D positional embedding style used by VGGT pooling helpers."""
    grid_h, grid_w = grid_hw
    img_h, img_w = img_hw
    if tokens.shape[1] != grid_h * grid_w:
        raise ValueError(
            f"Token count {tokens.shape[1]} does not match grid size {grid_h}x{grid_w}={grid_h * grid_w}."
        )

    pos_grid = create_uv_grid(grid_w, grid_h, aspect_ratio=img_w / img_h, dtype=tokens.dtype, device=tokens.device)
    pos_embed = position_grid_to_embed(pos_grid, tokens.shape[-1]).reshape(1, grid_h * grid_w, tokens.shape[-1])
    return tokens + ratio * pos_embed


class PerceiverResamplerLayer(nn.Module):
    """Flamingo-style latent-to-visual cross-attention block with a residual MLP."""
    ...
    def forward(self, latents: torch.Tensor, visual_tokens: torch.Tensor) -> torch.Tensor:
        ...
        kv = torch.cat([visual_tokens, latents], dim=1)
        kv = self.kv_norm(kv)
...
class PerceiverResampler(nn.Module):
    """Compress one image's VGGT patch tokens into a fixed grid of learned latent queries."""
    ...
    def __init__(
        ...
        *,
        ...
        num_heads: int = 8,
        mlp_ratio: float = 4.0,
        use_2d_positional_embedding: bool = True,
    ):
        super().__init__()
        latent_side = math.isqrt(num_latents)
        if latent_side * latent_side != num_latents:
            raise ValueError(f"num_latents must be a perfect square for 2D positional embedding, got {num_latents}.")

        self.num_latents = num_latents
        self.latent_side = latent_side
        self.use_2d_positional_embedding = use_2d_positional_embedding
        self.latents = nn.Parameter(torch.randn(num_latents, dim) * 0.02)
        self.layers = nn.ModuleList(
            [PerceiverResamplerLayer(dim, num_heads=num_heads, mlp_ratio=mlp_ratio) for _ in range(num_layers)]
        )
    ...
    def forward(
        ...
        *,
        ...
        if self.use_2d_positional_embedding:
            visual_tokens = _add_2d_positional_embedding(visual_tokens, patch_hw, img_hw)

        latents = self.latents[None, :, :].expand(visual_tokens.shape[0], -1, -1)
        if self.use_2d_positional_embedding:
            latents = _add_2d_positional_embedding(
                latents,
...
class PI0Pytorch(nn.Module):
    def __init__(self, config, extra_config):
        ...
        self.perceiver_num_layers = extra_config.perceiver_num_layers
        self.perceiver_num_latents = 256
        self.num_perceiver_resamplers = 2
        self.vggt_feature_dim = 2 * extra_config.vggt_dim
        self.vggt_resamplers = nn.ModuleList(
            [
                PerceiverResampler(
                    self.vggt_feature_dim,
                    num_latents=self.perceiver_num_latents,
                    num_layers=self.perceiver_num_layers,
                    use_2d_positional_embedding=bool(self.use_vggt_pe),
                )
                for _ in range(self.num_perceiver_resamplers)
            ]
        )
    ...
    def enable_perceiver_resampler_full_training(self) -> None:
        """Keep perceiver resamplers fully trainable even when the rest of the model uses LoRA-only updates."""
        for param in self.vggt_resamplers.parameters():
    ...
    def _force_float32_module(self, module):
        """Keep selected alignment-path modules in float32 regardless of outer AMP settings."""
        module_to_cast = (
            module.module if isinstance(module, torch.nn.parallel.DistributedDataParallel) else module
        )
        module_to_cast.to(dtype=torch.float32)
        return module

    def _resample_vggt_hidden(
        self,
        ...
        *,
        ...
        """Run one Perceiver Resampler per image stream and concatenate the latent outputs."""
        _, num_images, _, feature_dim = vggt_hidden.shape
        if num_images != len(self.vggt_resamplers):
            raise ValueError(
                f"Expected {len(self.vggt_resamplers)} image streams for perceiver resampling, got {num_images}."
            )
        if feature_dim != self.vggt_feature_dim:
            raise ValueError(f"Expected VGGT feature dim {self.vggt_feature_dim}, got {feature_dim}.")

        resampled = []
        for img_idx, resampler in enumerate(self.vggt_resamplers):
            resampled.append(resampler(vggt_hidden[:, img_idx], patch_hw=patch_hw, img_hw=img_hw))
    ...
    def forward(self, observation, actions, vggt, align_proj, noise=None, time=None) -> Tensor:
        ...
        # VGGT hidden states
        device_type = img_resize_wo_aug.device.type
        vggt = self._force_float32_module(vggt)
        self.vggt_resamplers.to(dtype=torch.float32)
        align_proj = self._force_float32_module(align_proj)
        with torch.autocast(device_type=device_type, enabled=False), torch.no_grad():
            vggt_output = vggt(img_resize_wo_aug.to(dtype=torch.float32))
        ...
        # Resample VGGT hidden states with one Perceiver Resampler per image stream.
        H, W = original_img.shape[-2:]
        patch_h, patch_w = H // vggt.patch_size, W // vggt.patch_size
        with torch.autocast(device_type=device_type, enabled=False):
            vggt_hidden = self._resample_vggt_hidden(
                vggt_hidden.to(dtype=torch.float32),
                patch_hw=(patch_h, patch_w),
                img_hw=(H, W),
            )
        if vggt_hidden.shape[1] != vision_hidden.shape[1]:
            raise ValueError(
                f"Perceiver output token count {vggt_hidden.shape[1]} must match VLA vision token count {vision_hidden.shape[1]}."
            )
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            align_loss = align_proj(
                vision_hidden.to(dtype=torch.float32),
                vggt_hidden.to(dtype=torch.float32),
                align_mask,
            )
