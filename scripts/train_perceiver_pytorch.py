...
Usage
Single GPU:
  python scripts/train_perceiver_pytorch.py <config_name> --exp_name <run_name> --save_interval <interval>
  Example:
  python scripts/train_perceiver_pytorch.py debug --exp_name pytorch_ddp_test
  python scripts/train_perceiver_pytorch.py debug --exp_name pytorch_ddp_test --resume  # Resume from latest checkpoint
Multi-GPU (single node):
  torchrun --standalone --nnodes=1 --nproc_per_node=<num_gpus> scripts/train_perceiver_pytorch.py <config_name> --exp_name <run_name>
  Example:
  torchrun --standalone --nnodes=1 --nproc_per_node=2 scripts/train_perceiver_pytorch.py pi0_aloha_sim --exp_name pytorch_ddp_test
  torchrun --standalone --nnodes=1 --nproc_per_node=2 scripts/train_perceiver_pytorch.py pi0_aloha_sim --exp_name pytorch_ddp_test --resume
Multi-Node Training:
	torchrun \
    --nnodes=<num_nodes> --nproc_per_node=<gpus_per_node> --node_rank=<rank_of_node> \
    --master_addr=<master_ip> --master_port=<port> \
    scripts/train_perceiver_pytorch.py <config_name> --exp_name=<run_name> --save_interval <interval>
...
from openpi.models_pytorch import pi0_pytorch, pi0_perceiver_pytorch, projectors
...
def train_loop(config: _config.TrainConfig):
    ...
    model = openpi.models_pytorch.pi0_perceiver_pytorch.PI0Pytorch(model_cfg, config).to(device)
    vggt_model = VGGT(
        enable_camera=False,
        enable_point=False,
        enable_depth=False,
    ...
    if config.lora_enabled:
        ...
        model.enable_perceiver_resampler_full_training()
        ...
        if is_main:
            ...
            perceiver_trainable = sum(p.numel() for p in model.vggt_resamplers.parameters() if p.requires_grad)
            total = sum(p.numel() for p in model.parameters())
            logging.info(
                f"Enabled LoRA: replaced {replaced} Linear layers, "
                f"trainable params={trainable}/{total}, "
                f"perceiver_trainable={perceiver_trainable}"
            )
