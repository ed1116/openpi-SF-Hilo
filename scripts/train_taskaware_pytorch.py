...
def train_loop(config: _config.TrainConfig):
    ...
    # [COPILOT] Coefficient for task-aware auxiliary objective: ITC + ITM + LM.
    ...
    running_taskaware_aux_loss = 0.0
        ...
        for observation, actions in loader:
            ...
            with no_sync_ctx:
                ...
                with torch.amp.autocast(device_type=device.type, enabled=use_amp, dtype=amp_dtype):
                    action_losses, align_loss, taskaware_aux_loss = model(
                        observation, actions, vggt=vggt_model, align_proj=align_projector
                    )
                    # [COPILOT] L_total = L_action + alpha * L_align + taskaware_loss_coeff * (ITC + ITM + LM).
                    loss = (
                        action_losses
                        + config.align_loss_coeff * align_loss
                        + taskaware_loss_coeff * taskaware_aux_loss
                    )
            ...
            running_taskaware_aux_loss += taskaware_aux_loss.item()
            accum_step += 1
            ...
            if is_main:
                ...
                avg_taskaware_aux_loss = running_taskaware_aux_loss / grad_accum_steps
                infos.append(
                    {
                        "action_loss": avg_action_loss,
                        "align_loss": avg_align_loss,
                        "taskaware_aux_loss": avg_taskaware_aux_loss,
            ...
            if is_main and (global_step % config.log_interval == 0):
                ...
                avg_taskaware_aux_loss = sum(info["taskaware_aux_loss"] for info in infos) / len(infos)
                ...
                logging.info(
                    f"step={global_step} action_loss={avg_loss:.4f} align_loss={avg_align_loss:.4f} taskaware_aux_loss={avg_taskaware_aux_loss:.4f} lr={avg_lr:.2e} grad_norm={avg_grad_norm:.2f} time={elapsed:.1f}s"
                    if avg_grad_norm is not None
                    else f"step={global_step} action_loss={avg_loss:.4f} align_loss={avg_align_loss:.4f} taskaware_aux_loss={avg_taskaware_aux_loss:.4f} lr={avg_lr:.2e} time={elapsed:.1f}s"
                )
            ...
            if pbar is not None:
                ...
                weighted_align_loss = config.align_loss_coeff * avg_align_loss
                weighted_task_loss = taskaware_loss_coeff * avg_taskaware_aux_loss
                total_loss = avg_action_loss + weighted_align_loss + weighted_task_loss
                pbar.set_postfix(
                    {
                        "loss": f"{total_loss:.4f}",
                        "action_loss": f"{avg_action_loss:.4f}",
