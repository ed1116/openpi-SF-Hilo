...
MAIN_PY="${ROOT_DIR}/examples/libero-pro/main_copy.py"
...
SEED="${SEED:-7}"
VIDEO_ROOT="${VIDEO_ROOT:-${ROOT_DIR}/data/libero-pro-copy-9000/videos}"
LOG_ROOT="${LOG_ROOT:-${ROOT_DIR}/data/libero-pro-copy-9000/logs}"
DRY_RUN="${DRY_RUN:-0}"
...
if [[ ! -f "${MAIN_PY}" ]]; then
  echo "[ERROR] main_copy.py not found: ${MAIN_PY}"
  exit 1
fi
